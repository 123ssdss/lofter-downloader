...
from config import (
    ...
    REQUEST_TIMEOUT,
    TEXT_MAX_WORKERS,
    USER_COOKIE,  # Import user-provided cookie from config
)
...
def _format_comment(comment, indent_level=0):
    ...
    publish_time = datetime.fromtimestamp(
        comment.get("publishTime", 0) / 1000
    ).strftime("%Y-%m-%d %H:%M:%S")
    content = comment.get("content", "").strip()

    return f"{indent}Author: {author} ({publish_time})\n{indent}Content: {content}\n"
...
class LofterClient:
    ...
    def _make_request(
        self, method, url, params=None, data=None, headers=None, max_retries=3
    ):
        ...
        try:
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    )  # Limit body output to prevent too much text
                    ...
                    if (
                        ...
                        and json_response.get("code") == 500
                    ):
                        error_msg = json_response.get("msg", "Unknown error")
                        self._log(
                            f"API error: {error_msg} (attempt {attempt + 1}/{max_retries})"
                        )
    ...
    def fetch_posts_by_tag(self, tag, list_type=None, timelimit=None, blog_type=None):
        ...
        from config import DEFAULT_BLOG_TYPE, DEFAULT_LIST_TYPE, DEFAULT_TIME_LIMIT
        ...
        while True:
            ...
            print(
                f"\033[32m[INFO]\033[0m LofterClient 获取标签 '{tag}': {len(all_posts)} 个帖子"
            )
            time.sleep(BETWEEN_PAGES_DELAY)

        print(
            f"\033[32m[INFO]\033[0m LofterClient 标签 '{tag}' 获取完成: {len(all_posts)} 个独特帖子"
        )
    ...
    def fetch_post_detail(self, post_meta):
        ...
        self._log(
            f"Fetching post detail ID {post_view['id']} from blog {blog_info['blogName']}"
        )
    ...
    def fetch_post_detail_by_id(self, post_id, blog_id):
        ...
        # 尝试获取博客域名
        blog_info_response = self._make_request(
            "GET",
            f"{API_BASE_URL}/v1.1/bloginfo.api",
            params={"product": "lofter-android-7.9.7.2", "blogids": str(blog_id)},
        )

        blog_domain = f"{blog_id}.lofter.com"
        if (
            blog_info_response
            and "response" in blog_info_response
            and "blogs" in blog_info_response["response"]
            and len(blog_info_response["response"]["blogs"]) > 0
        ):
    ...
    def _fetch_l2_comments(self, post_id, blog_id, comment_id, max_retries=2):
        ...
        for attempt in range(max_retries):
            ...
            if (
                ...
                self._log(
                    f"Retrying L2 request for comment {comment_id} in {wait_time}s (attempt {attempt + 1}/{max_retries})"
                )
                time.sleep(wait_time)
                continue
    ...
    def fetch_all_comments_for_post(
        ...
        """获取帖子的所有评论，包括回复，修复重复评论的问题"""
        for attempt in range(max_retries):
            try:
                self._log(
                    f"Fetching comments for post {post_id} (attempt {attempt + 1}/{max_retries})"
                )
                offset = 0
                all_hot_comments = []  # 单独存储热门评论
                all_normal_comments = []  # 单独存储普通评论
                seen_comment_ids = set()  # 全局去重ID集合
                page = 1
                ...
                while True:
                    ...
                    if (
                        ...
                        self._log(
                            f"Failed to fetch L1 comments for post {post_id} (page {page})"
                        )
                        break
                    ...
                    # 获取当前页的热门评论和普通评论
                    page_hot_comments = response["data"].get("hotList", [])
                    page_normal_comments = response["data"].get("list", [])

                    # 处理热门评论，确保没有重复
                    for comment in page_hot_comments:
                        comment_id = comment.get("id")
                        if comment_id and comment_id not in seen_comment_ids:
                            comment["is_hot_comment"] = True
                            all_hot_comments.append(comment)
                            seen_comment_ids.add(comment_id)
                            self._log(f"Added unique hot comment ID: {comment_id}")

                    # 处理普通评论，确保没有重复且不在热门评论中
                    for comment in page_normal_comments:
                        comment_id = comment.get("id")
                        if comment_id and comment_id not in seen_comment_ids:
                            comment["is_hot_comment"] = False
                            all_normal_comments.append(comment)
                            seen_comment_ids.add(comment_id)
                            self._log(f"Added unique normal comment ID: {comment_id}")

                    # 记录评论统计
                    self._log(
                        f"Page {page}: {len(page_hot_comments)} hot + {len(page_normal_comments)} normal"
                    )
                    self._log(
                        f"Total collected: {len(all_hot_comments)} hot + {len(all_normal_comments)} normal"
                    )
                ...
                # 创建修正后的评论结构
                # all_list应该包含所有评论(热门+普通)，但需要正确去重
                all_comments = all_hot_comments + all_normal_comments

                structured_comments = {
                    "hot_list": all_hot_comments,  # 仅热门评论
                    "all_list": all_comments,  # 所有评论，但已去重
                }
                ...
                if return_structure:
                    ...
                    with concurrent.futures.ThreadPoolExecutor(
                        max_workers=COMMENT_MAX_WORKERS
                    ) as executor:
                        future_to_comment = {
                            executor.submit(
                                self._process_l1_comment_with_replies,
                                post_id,
                                blog_id,
                                c,
                            ): c
                            for c in structured_comments["hot_list"]
                            if "id" in c
                        }
                    ...
                    with concurrent.futures.ThreadPoolExecutor(
                        ...
                        future_to_comment = {
                            ...
                            for c in structured_comments["all_list"]
                            if "id" in c
                        }
                    ...
                    self._save_original_responses(
                        post_id, blog_id, structured_result, mode, name
                    )
                        ...
                        for future in concurrent.futures.as_completed(
                            ...
                            try:
                                ...
                                if result:  # Only add if we have a result
                                    hot_comments_text += (
                                        self._format_comment_with_replies_text(result)
                                    )
                            except Exception as e:
                                self._log(f"Error processing hot comment: {str(e)}")
                ...
                self._log(
                    f"Error fetching comments for post {post_id} (attempt {attempt + 1}/{max_retries}): {e}"
                )
                if attempt < max_retries - 1:
                    wait_time = (
                        attempt + 1
                    ...
                    self._log(
                        f"All retry attempts failed for post {post_id}. Returning empty result."
                    )
                    return "" if not return_structure else []

    def _save_original_responses(
        self, post_id, blog_id, structured_result, mode="comment", name=""
    ):
        ...
        try:
            ...
            if mode == "blog":
                ...
                json_dir = path_manager.get_json_dir(
                    mode, name or "default_tag_name", "comments"
                )
                filename = f"comments_{post_id}_{blog_id}.json"
            elif mode == "collection":
                # json/collection/collection名字/comments
                json_dir = path_manager.get_json_dir(
                    mode, name or "default_collection_name", "comments"
                )
                filename = f"comments_{post_id}_{blog_id}.json"
            elif mode == "comment":
                # json/comments
            ...
            # 同时保存为用户要求的格式
            self._save_comments_in_user_format(
                post_id, blog_id, structured_result, mode, name
            )
        except Exception as e:
            self._log(f"Error saving original responses: {str(e)}")

    def _save_comments_in_user_format(
        self, post_id, blog_id, structured_result, mode="comment", name=""
    ):
        ...
        try:
            ...
            with open(filepath, "w", encoding="utf-8") as f:
                ...
                for comment in all_comments:
                    # 写入主评论信息
                    f.write(
                        f"发布人：{comment.get('author', {}).get('blogNickName', 'Unknown')}\n"
                    )
                    ...
                    if replies:
                        ...
                        for idx, reply in enumerate(replies, 1):
                            f.write(f"回复{idx}：\n")
                            f.write(
                                f"  作者：{reply.get('author', {}).get('blogNickName', 'Unknown')}\n"
                            )
                            f.write(
                                f"  时间：{reply.get('publishTimeFormatted', '')}\n"
                            )
    ...
    def _process_l1_comment_with_replies(self, post_id, blog_id, l1_comment):
        ...
        normalized_replies = [
            self._normalize_comment_format(reply, "L2")
            for reply in embedded_l2_comments
        ]
        ...
        if l2_count > len(embedded_l2_comments):
            ...
            if l2_data is None:
                ...
                self._log(
                    f"L2 response is string for comment {comment_id}: {l2_data[:100]}..."
                )
            elif not isinstance(l2_data, dict):
                self._log(
                    f"L2 response is not a dict for comment {comment_id}: {type(l2_data)}"
                )
                ...
                self._log(
                    f"L2 request failed for comment {comment_id}: Status {status_code}, Message: {message}"
                )
                ...
                self._log(
                    f"L2 API error for comment {comment_id}: Code {error_code}, Message: {error_msg}"
                )
            else:
                self._log(
                    f"Successfully retrieved L2 comments for L1 comment {comment_id}"
                )
                ...
                if additional_l2_comments:
                    ...
                    for l2_comment in additional_l2_comments:
                        ...
                        if any(
                            emb.get("id") == l2_comment.get("id")
                            for emb in embedded_l2_comments
                        ):
                            continue
                        normalized_replies.append(
                            self._normalize_comment_format(l2_comment, "L2")
                        )

                    # Check if we found all expected replies
                    total_found = len(embedded_l2_comments) + len(
                        [
                            r
                            for r in normalized_replies
                            if r
                            not in [
                                self._normalize_comment_format(emb, "L2")
                                for emb in embedded_l2_comments
                            ]
                        ]
                    )
                    if total_found < l2_count:
                        self._log(
                            f"Warning: Found {total_found} replies but expected {l2_count} for comment {comment_id}"
                        )
                else:
                    self._log(
                        f"No additional L2 comments found in response for comment {comment_id}"
                    )
    ...
    def get_collection_list(self, collection_id, offset=0, limit=15):
        ...
        try:
            ...
            response = requests.post(
                COLLECTION_URL, params=params, data=payload, headers=headers, timeout=15
            )
    ...
    def get_subs(self, offset=0, limit_once=50):
        ...
        params = {"offset": offset, "limit": limit_once}

        headers = FIXED_HEADERS.copy()

        # Make the request
        response = self._make_request(
            "GET", SUBSCRIPTION_URL, params=params, headers=headers
        )
        ...
        if response and response.get("code") == 0:  # Successful response
            ...
            if response:
                ...
                self._log(
                    "Failed to fetch subscription list: Request failed or no response"
                )
            return None

    def save_subscription_list(
        self, auth_info, save_path="./results", sleep_time=0.1, limit_once=50
    ):
        """
    ...
    def fetch_html_content(self, url, timeout=30):
        ...
        try:
            ...
            response = requests.get(
                url, cookies=cookies, headers=browser_headers, timeout=timeout
            )
