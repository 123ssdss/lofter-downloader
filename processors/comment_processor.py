...
class CommentProcessor(ContentProcessor):
    ...
    def format_single_comment(
        self, comment: Dict[str, Any], indent_level: int = 0, is_reply: bool = False
    ) -> str:
        ...
        if is_reply:
            ...
            result += (
                f"{indent}发布人：{author}{f'[{blog_name}]' if blog_name else ''}\n"
            )
    ...
    def format_comments_recursive(
        self, comments_data: List[Dict[str, Any]], indent_level: int = 0
    ) -> str:
        """递归格式化评论和回复"""
        result = ""
    ...
    def process_comments_data(self, structured_comments: Dict[str, Any]) -> str:
        ...
        try:
            # 检查返回的数据结构是新的还是旧的
            if (
                isinstance(structured_comments, dict)
                and "hot_list" in structured_comments
                and "all_list" in structured_comments
            ):
                # 添加最终去重检查 - 防御性编程
                unique_all_list = []
                seen_ids = set()

                for comment in structured_comments["all_list"]:
                    comment_id = comment.get("id")
                    if comment_id not in seen_ids:
                        unique_all_list.append(comment)
                        seen_ids.add(comment_id)

                # 使用去重后的列表
                result = "[热门评论]\n"
                result += self.format_comments_recursive(
                    structured_comments["hot_list"]
                )
                result += "\n[全部评论]\n"
                result += self.format_comments_recursive(unique_all_list)
                return result
            else:
                # 旧结构: 直接处理列表
                # 检查是否是列表，如果不是，尝试从字典中提取列表
                if isinstance(structured_comments, list):
                    return self.format_comments_recursive(structured_comments)
                elif isinstance(structured_comments, dict):
                    # 尝试从字典中提取评论列表
                    for key in ["list", "comments", "data"]:
                        if key in structured_comments and isinstance(
                            structured_comments[key], list
                        ):
                            return self.format_comments_recursive(
                                structured_comments[key]
                            )
                    # 如果找不到列表，返回空字符串
                    return ""
                else:
    ...
    def process_post_comments(
        self, post_id: str, blog_id: str, mode: str = "comment", name: str = ""
    ) -> str:
        """获取并格式化帖子的所有评论"""
        try:
            # 获取结构化评论数据
    ...
    def save_comments_data(
        self, comments_text: str, mode: str, name: str, base_filename: str
    ) -> str:
        """保存评论数据到文件"""
        try:
            comments_dir = path_manager.get_json_dir(mode, name, "comments")
            comments_file_path = os.path.join(
                comments_dir, f"{base_filename}_comments.txt"
            )
